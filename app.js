...
function openRoomSettings(rid) {
    ...
    document.getElementById('mContent').innerHTML = `
        ...
        <div class="flex-row" style="margin-bottom: 15px;">
            ...
            <div class="flex-col"><label style="font-size: 12px; font-weight: bold; margin-bottom: 6px; display: block;">Giá gốc / đêm</label><input class="f-group" style="margin-bottom:0;" id="fRoomPrice" type="text" inputmode="numeric" value="${numToStr(r.price)}" oninput="formatCur(this)"></div>
        </div>
...
function openBooking(roomId, date) {
    ...
    document.getElementById('mContent').innerHTML = `
        ...
        <div class="flex-row" style="margin-bottom: 15px;">
            ...
            <div class="flex-col"><label style="font-size: 12px; font-weight: bold; margin-bottom: 6px; display: block;">Tiền đã cọc (VNĐ)</label><input class="f-group" style="margin-bottom:0;" id="fDeposit" type="text" inputmode="numeric" value="0" oninput="formatCur(this); checkDepositStatus();" placeholder="0"></div>
        </div>
        <div class="f-group">
        ...
        <div style="background:#fdfaf0; border: 1px solid #fcebb6; padding:15px; border-radius:12px; margin-bottom:15px;">
            ...
            <div id="mbList" style="font-size: 14px; color: #444;">
                ${miniItems.map((m, i) => `<div style="display:flex; justify-content:space-between; padding: 6px 0; border-bottom: 1px dashed #e0e0e0;">
                    ...
                    <span><b>${numToStr(m.qty * m.price)}đ</b> <span style="color:var(--p-red); cursor:pointer; margin-left:12px; font-weight:bold;" onclick="removeMb('${bid}', ${i})">✕</span></span>
                </div>`).join('')}
            </div>
        </div>`;
    }
    ...
    if (isCompleted) {
        ...
        document.getElementById('mContent').innerHTML = `
            ...
            <div style="background:#f0f7ff; padding:15px; border-radius:12px; margin-bottom:15px;">
                ...
                <div class="flex-row">
                    <div class="flex-col"><label style="font-size: 12px; font-weight: bold; margin-bottom: 6px; display: block;">Giá / Đêm</label><input class="f-group" style="margin-bottom:0;" id="dPrice" type="text" inputmode="numeric" value="${numToStr(appliedPrice)}" oninput="formatCur(this)"></div>
                    <div class="flex-col"><label style="font-size: 12px; font-weight: bold; margin-bottom: 6px; display: block;">Tiền cọc</label><input class="f-group" style="margin-bottom:0;" id="dDeposit" type="text" inputmode="numeric" value="${numToStr(deposit)}" oninput="formatCur(this)"></div>
                </div>
...
async function addRoom() {
    ...
    if(id && price) { 
        await _supabase.from('rooms').insert([{ id, price: getNum(price), cat: "", status: "clean", lockbox: "", notes: "" }]);
    }
}

// --- TÍNH NĂNG XUẤT EXCEL DẠNG TIMELINE (GANTT CHART) ---
    ...
    rooms.forEach(r => {
        ...
        while(d <= daysInMonth) {
            ...
            if(b) {
                ...
                let totalNights = Math.max((new Date((b.end?b.end.substring(0,10):realTodayStr)+'T00:00:00') - new Date((b.start?b.start.substring(0,10):realTodayStr)+'T00:00:00')) / 86400000, 1);
                let miniTotal = parseInt(b.minibar_total) || 0;
                let totalCost = (totalNights * appliedPrice) + miniTotal;
                let dep = b.deposit || 0;
